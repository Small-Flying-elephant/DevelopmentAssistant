...
public class PermissionUtil {
     ...
     *
     * @param context
     * @param serviceName 服务名称（名称可参考{@link
     */
    public static boolean getServiceState(Context context, String serviceName) {
        ...
        if (enable == 1) {
            ...
            while (splitter.hasNext()) {
                ...
                Log.d("aiman", "getServiceState: " + accessibilityService);
                Log.d("aiman", "serviceName:     " + serviceName);
                if (accessibilityService.equals(serviceName)) {
                    Log.d("aiman", "getServiceState: " + serviceName);
                    return true;
                }
            }
            return false;
